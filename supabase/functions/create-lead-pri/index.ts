...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Upsert via RPC oficial (deduplicação + vínculo automático em eventos_prospeccao)
    ...
    // 3. Buscar contato resultante por telefone+empresa
    ...
    // 4. Verificar/garantir vínculo (bulk_upsert_contatos já faz IF NOT EXISTS)
    const { data: vinculoExistente } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    const vinculo_criado = !!vinculoExistente; // já existe após o upsert

    // 5. Setar origem_pri (preservando valor antigo) e atribuir Pri IA se status ainda for "Novo"
