...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'usuario_tem_cadeira_externa', EXISTS (
      ...
      WHERE eas.prospeccao_id = scoped_prospeccoes.id
        ...
        AND eas.profile_id = user_id_param
  ...
  WITH candidates AS (
    ...
    SELECT
      COUNT(*) FILTER (WHERE status_evento = 'Novo')::integer AS novos_total,
      COUNT(*) FILTER (WHERE motivo = 'elegivel_pela_rpc_atual')::integer AS elegiveis_total
    FROM classified
  ), reasons AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object('motivo', motivo, 'total', total) ORDER BY motivo), '[]'::jsonb) AS bloqueios
    FROM (
  ...
  SELECT stats.novos_total, stats.elegiveis_total, reasons.bloqueios
  INTO v_novos_total, v_elegiveis_total, v_bloqueios
  FROM stats CROSS JOIN reasons;
...
REVOKE ALL ON FUNCTION public.debug_auto_atribuicao_leads(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.debug_auto_atribuicao_leads(uuid, uuid) FROM anon;
