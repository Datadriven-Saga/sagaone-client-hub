...
-- Update get_kanban_columns to include tentativas_chamada
CREATE OR REPLACE FUNCTION public.get_kanban_columns(p_empresa_id uuid, p_per_column integer DEFAULT 20, p_prospeccao_ids uuid[] DEFAULT NULL::uuid[], p_responsavel text DEFAULT NULL::text, p_search text DEFAULT NULL::text, p_date_start timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_end timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
...
-- Update get_kanban_columns_limited to include tentativas_chamada in all SELECT statements
-- First, get the full function and recreate it
CREATE OR REPLACE FUNCTION public.get_kanban_columns_limited(p_empresa_id uuid, p_per_column integer DEFAULT 20, p_prospeccao_ids uuid[] DEFAULT NULL::uuid[], p_responsavel text DEFAULT NULL::text, p_search text DEFAULT NULL::text, p_date_start timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_end timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
  ...
  LOOP
    ...
    ELSE
      IF v_has_filter THEN
        ...
        WHERE c.empresa_id = p_empresa_id
          AND c.status::text = v_status
          AND (c.responsavel_email = v_user_email OR c.vendedor_nome = v_user_nome OR c.vendedor_nome = v_user_email)
          AND (p_date_start IS NULL OR c.created_at >= p_date_start)
          AND (p_date_end IS NULL OR c.created_at <= p_date_end)
          AND (p_search IS NULL OR 
               c.nome ILIKE '%' || p_search || '%' OR 
...
-- Update get_contatos_paginated to include tentativas_chamada
CREATE OR REPLACE FUNCTION public.get_contatos_paginated(p_empresa_id uuid, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_prospeccao_ids uuid[] DEFAULT NULL::uuid[], p_status text DEFAULT NULL::text, p_responsavel text DEFAULT NULL::text, p_search text DEFAULT NULL::text, p_sort_column text DEFAULT 'updated_at'::text, p_sort_direction text DEFAULT 'desc'::text, p_date_start timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_end timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
