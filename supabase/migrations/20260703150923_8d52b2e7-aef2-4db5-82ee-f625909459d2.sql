CREATE OR REPLACE FUNCTION public.bulk_upsert_contatos(
  ...
  p_strict_responsavel boolean DEFAULT false
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  LOOP
    BEGIN
      ...
      IF v_responsavel IS NOT NULL THEN
        ...
        BEGIN
          -- FIX: aceitar usuário via profiles.empresa_id OU vínculo em user_empresas (multi-tenant)
      ...
      IF v_responsavel IS NOT NULL AND v_responsavel <> '' THEN
        BEGIN
          -- FIX: mesmo predicado multi-tenant no UPDATE do vendedor_nome
          ...
          -- FIX: mesmo predicado multi-tenant no EXISTS de confirmação
          IF EXISTS (
            SELECT 1
            FROM public.profiles p
            LEFT JOIN auth.users au ON au.id = p.id
