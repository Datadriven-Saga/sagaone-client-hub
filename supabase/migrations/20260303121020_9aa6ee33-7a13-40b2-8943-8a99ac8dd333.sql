...
-- 1. Soft-delete columns on contato_quarentena
...
-- 2. Audit trail table
CREATE TABLE public.quarentena_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  quarentena_id uuid REFERENCES public.contato_quarentena(id) ON DELETE SET NULL,
...
ALTER TABLE public.quarentena_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins can view quarentena logs"
  ON public.quarentena_logs FOR SELECT
  TO authenticated
  USING (public.check_user_is_admin());

CREATE POLICY "Admins can insert quarentena logs"
  ON public.quarentena_logs FOR INSERT
  TO authenticated
  WITH CHECK (public.check_user_is_admin());

-- 3. Test flag on prospeccoes
ALTER TABLE public.prospeccoes 
  ADD COLUMN IF NOT EXISTS is_teste boolean NOT NULL DEFAULT false;

-- 4. Update bulk_upsert_contatos to skip quarantine for test prospections and respect desativado
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF v_marca IS NOT NULL AND v_contato_id IS NOT NULL AND NOT v_is_teste THEN
...
-- 5. Update check_quarentena to respect desativado flag
CREATE OR REPLACE FUNCTION public.check_quarentena(p_telefones text[], p_loja_id uuid)
