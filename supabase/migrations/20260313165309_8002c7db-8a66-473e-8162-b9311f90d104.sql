...
-- Table for phone numbers that should never be quarantined (test contacts)
CREATE TABLE public.quarentena_exclusoes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  telefone_normalizado text NOT NULL UNIQUE,
  motivo text DEFAULT 'teste',
  criado_por uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.quarentena_exclusoes ENABLE ROW LEVEL SECURITY;

-- Only admins can manage exclusions
CREATE POLICY "Admins can manage quarentena_exclusoes"
  ON public.quarentena_exclusoes
  FOR ALL
  TO authenticated
  USING (public.check_user_is_admin(auth.uid()))
  WITH CHECK (public.check_user_is_admin(auth.uid()));

-- Insert the 6 test phone numbers
INSERT INTO public.quarentena_exclusoes (telefone_normalizado, motivo) VALUES
  ('62999001697', 'Contato de teste'),
  ('62999199312', 'Contato de teste'),
  ('62999205245', 'Contato de teste'),
  ('62999016844', 'Contato de teste'),
  ('62999237569', 'Contato de teste'),
  ('62999310242', 'Contato de teste');

-- Delete all quarantine records for these test phones
DELETE FROM public.contato_quarentena
WHERE telefone_normalizado IN ('62999001697', '62999199312', '62999205245', '62999016844', '62999237569', '62999310242');

-- Update bulk_upsert_contatos to skip excluded phones
...
-- Also update upsert_quarentena to respect exclusions
CREATE OR REPLACE FUNCTION public.upsert_quarentena(p_telefone text, p_loja_id uuid, p_prospeccao_id uuid, p_evento_nome text, p_data_fim_evento timestamp with time zone)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
