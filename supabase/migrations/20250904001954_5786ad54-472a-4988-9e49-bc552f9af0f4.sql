...
-- 1. Administrators can access all clients in their company
CREATE POLICY "Administrators can access all company clients"
ON public.clientes
FOR ALL
TO authenticated
USING (
  ...
  AND (
    get_current_user_access_type() = 'Administrador'::tipo_acesso
    OR get_current_user_access_type() = 'TI'::tipo_acesso
    OR get_current_user_access_type() = 'Gerente'::tipo_acesso
  )
)
WITH CHECK (
  empresa_id = get_user_active_company()
...
-- 2. Regular users (SDR, Vendedor) can only access clients they own or are assigned to
...
AS $$
  ...
  AND (
    -- Administrators, TI, and Managers can access all company clients
    ui.tipo_acesso = ANY(ARRAY['Administrador'::tipo_acesso, 'TI'::tipo_acesso, 'Gerente'::tipo_acesso])
    OR
