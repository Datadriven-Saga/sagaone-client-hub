...
CREATE OR REPLACE FUNCTION public.preview_restauracao_por_perfil(
  p_empresa_id uuid,
  p_tipo_acesso text[] DEFAULT ARRAY['Vendedor'::text],
...
BEGIN
  ...
  ultimo_log AS (
    ...
    WHERE l.contato_id IS NOT NULL
      ...
      AND (p_data_de IS NULL OR l.data_movimentacao >= p_data_de)
      AND (p_data_ate IS NULL OR l.data_movimentacao <= p_data_ate)
  ...
  divergentes AS (
    SELECT c.id AS contato_id, c.nome, c.status::text AS status_atual,
           u.status_esperado, u.usuario_id, u.log_email, u.data_movimentacao,
