CREATE OR REPLACE FUNCTION public.bulk_upsert_contatos(p_contatos jsonb, p_empresa_id uuid, p_prospeccao_id uuid DEFAULT NULL::uuid, p_canal text DEFAULT 'whatsapp'::text, p_force_status_novo boolean DEFAULT false, p_strict_responsavel boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_skipped_responsavel_invalido int := 0;
  v_contato record;
  v_contato_id uuid;
  v_is_new boolean;
  v_marca text;
  ...
  v_responsavel_original text;
  v_responsavel_resolved text;
BEGIN
  v_quarentena_enabled := public.is_feature_enabled('quarentena_marca_ativa');

  SELECT e.marca, COALESCE(e.bypass_compliance, false)
  INTO v_marca, v_bypass
  FROM public.empresas e
  WHERE e.id = p_empresa_id;

  IF v_bypass THEN
    v_quarentena_enabled := false;
  END IF;
  ...
  LOOP
    BEGIN
      ...
      IF v_responsavel IS NOT NULL THEN
        v_responsavel_original := v_responsavel;
        v_responsavel_resolved := NULL;
        BEGIN
          SELECT lower(au.email)
          INTO v_responsavel_resolved
          ...
          WHERE (
                  ...
                  OR EXISTS (
                    ...
                    WHERE ue.user_id = p.id
                      ...
                      AND COALESCE(ue.is_ativa, true) = true
                  )
                )
            AND COALESCE(p.is_active, true) = true
            AND (
              lower(au.email) = lower(v_responsavel_original)
              OR p.id::text = v_responsavel_original
              OR p.celular = v_responsavel_original
            )
          LIMIT 1;

          IF v_responsavel_resolved IS NULL THEN
            IF p_strict_responsavel THEN
              v_skipped_responsavel_invalido := v_skipped_responsavel_invalido + 1;
              IF jsonb_array_length(v_error_details) < 500 THEN
                v_error_details := v_error_details || jsonb_build_object(
                  'row', v_row_index,
                  'telefone', v_telefone_raw,
                  'nome', v_nome_raw,
                  'responsavel_email', v_responsavel_original,
                  'motivo', 'responsavel_inexistente',
                  'erro', 'Responsável "' || v_responsavel_original || '" não existe no sistema. Deixe em branco para distribuição ou informe um email válido.'
                );
              END IF;
              CONTINUE;
            END IF;
            ...
            END IF;
          ELSE
            v_responsavel := v_responsavel_resolved;
          END IF;
      ...
      DO UPDATE SET
        ...
        responsavel_email = CASE
          -- 1) Reset explícito COM responsável no CSV: aplica sempre
          ...
          -- 2) Reset explícito SEM responsável: só limpa se contato não estiver em outro evento ativo
          WHEN p_force_status_novo AND NOT EXISTS (
            SELECT 1
            FROM public.eventos_prospeccao ep
            JOIN public.prospeccoes pr ON pr.id = ep.prospeccao_id
            WHERE ep.contato_id = contatos.id
              AND (p_prospeccao_id IS NULL OR ep.prospeccao_id <> p_prospeccao_id)
              AND COALESCE(pr.ativo, true) = true
          )
            THEN NULL
          -- 3) CSV trouxe responsável (import comum): aplica só se atual estiver vazio (preserva responsável existente)
          WHEN EXCLUDED.responsavel_email IS NOT NULL AND EXCLUDED.responsavel_email <> ''
            THEN COALESCE(NULLIF(contatos.responsavel_email, ''), EXCLUDED.responsavel_email)
          ELSE contatos.responsavel_email
        END,
        status = CASE
          -- 1) Reset explícito COM responsável no CSV: vai para Atribuído (comportamento pré-existente)
          WHEN p_force_status_novo AND EXCLUDED.responsavel_email IS NOT NULL AND EXCLUDED.responsavel_email <> ''
            THEN 'Atribuído'::status_lead
          -- 2) Reset explícito SEM responsável: só regride para "Novo" se contato não estiver em outro evento ativo
          WHEN p_force_status_novo AND NOT EXISTS (
            SELECT 1
            FROM public.eventos_prospeccao ep
            JOIN public.prospeccoes pr ON pr.id = ep.prospeccao_id
          ...
          -- 3) CSV trouxe responsável e status atual ainda é Novo: promove para Atribuído
          WHEN EXCLUDED.responsavel_email IS NOT NULL AND EXCLUDED.responsavel_email <> '' AND contatos.status = 'Novo'::status_lead
            THEN 'Atribuído'::status_lead
      ...
      ELSIF p_force_status_novo THEN
        -- Só limpa vendedor_nome se estamos efetivamente resetando este contato
        -- (mesma proteção contra contaminação de eventos irmãos)
        UPDATE public.contatos
        SET vendedor_nome = NULL
        WHERE id = v_contato_id
          AND NOT EXISTS (
            SELECT 1
            FROM public.eventos_prospeccao ep
            JOIN public.prospeccoes pr ON pr.id = ep.prospeccao_id
            WHERE ep.contato_id = v_contato_id
              AND (p_prospeccao_id IS NULL OR ep.prospeccao_id <> p_prospeccao_id)
              AND COALESCE(pr.ativo, true) = true
          );
      END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_responsavel_invalido', v_skipped_responsavel_invalido,
    'warning_details', v_warning_details,
    'errors', v_errors,
    'error_details', v_error_details,
    'total', jsonb_array_length(p_contatos),
    'bypass_compliance', v_bypass
  );
END;
$function$;