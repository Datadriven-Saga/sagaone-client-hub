-- 1. Add valor JSONB column to departamento_permissoes
ALTER TABLE public.departamento_permissoes
ADD COLUMN IF NOT EXISTS valor JSONB DEFAULT NULL;

COMMENT ON COLUMN public.departamento_permissoes.valor IS
'Configuração extra da permissão. Ex: {"dias_max": 180} para canImportPool. null em dias_max = ilimitado.';

-- 2. Seed valor para canImportPool (sem limite para Admin/Master/CRM/TI; 30 dias para demais)
-- Insere linhas faltantes
INSERT INTO public.departamento_permissoes (departamento, permissao, ativo, valor)
SELECT t, 'canImportPool',
  CASE WHEN t IN ('Master','Administrador','TI','CRM') THEN true ELSE false END,
  CASE WHEN t IN ('Master','Administrador','TI','CRM')
       THEN '{"dias_max": null}'::jsonb
       ELSE '{"dias_max": 30}'::jsonb END
FROM (VALUES
  ('SDR'),('Vendedor'),('CRM'),('Recepcionista'),
  ('Gerente de Leads'),('Gerente de Loja'),('Coordenadora de Leads'),
  ('Diretor'),('TI'),('Administrador'),('Proprietário'),('Master')
) AS r(t)
ON CONFLICT (departamento, permissao) DO NOTHING;

-- Atualiza valor onde estava nulo
UPDATE public.departamento_permissoes
SET valor = CASE
  WHEN departamento IN ('Master','Administrador','TI','CRM')
    THEN '{"dias_max": null}'::jsonb
  ELSE '{"dias_max": 30}'::jsonb
END
WHERE permissao = 'canImportPool' AND valor IS NULL;

-- 3. Atualiza get_pool_facets_for_empresa para retornar data_min/data_max
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'data_min', (SELECT MIN(criado_em_origem) FROM base),
    'data_max', (SELECT MAX(criado_em_origem) FROM base),
...
-- 4. Atualiza get_pool_clientes_for_empresa para suportar dias_atras
