...
BEGIN
  ...
  WHERE ep.prospeccao_id = p_prospeccao_id
    ...
    AND (
      p_status = 'todos'
      OR (p_status = 'pendente' AND ep.status::text = 'pendente')
      OR (p_status = 'disparado' AND ep.status::text = 'disparado')
    )
    AND (
      p_disparo = 'todos'
...
GRANT EXECUTE ON FUNCTION public.export_evento_base(uuid, uuid, uuid, int, text, text, text) TO authenticated;