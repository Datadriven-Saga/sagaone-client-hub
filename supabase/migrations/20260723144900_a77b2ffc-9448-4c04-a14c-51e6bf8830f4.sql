...
BEGIN
  ...
  LOOP
    ...
    ELSE
      IF v_has_filter THEN
        SELECT COALESCE(t.cnt, 0), COALESCE(i.items, '[]'::jsonb) INTO v_count, v_items
        FROM (
        ...
        LEFT JOIN LATERAL (
          SELECT status, COALESCE(jsonb_agg(row_to_json(sub2.*) ORDER BY updated_at DESC, id), '[]'::jsonb) AS items
          FROM (
