CREATE OR REPLACE FUNCTION public.get_leads_status_divergente(p_empresa_ids uuid[] DEFAULT NULL::uuid[], p_prospeccao_ids uuid[] DEFAULT NULL::uuid[], p_status_atual text[] DEFAULT NULL::text[], p_status_esperado text[] DEFAULT NULL::text[], p_search text DEFAULT NULL::text, p_data_de timestamp with time zone DEFAULT NULL::timestamp with time zone, p_data_ate timestamp with time zone DEFAULT NULL::timestamp with time zone, p_page integer DEFAULT 1, p_page_size integer DEFAULT 50)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  SELECT EXISTS (
    ...
    WHERE pr.id = v_uid
      AND (pr.tipo_acesso IN ('Administrador','TI','Master') OR pr.is_master = true)
  ) INTO v_is_admin;
