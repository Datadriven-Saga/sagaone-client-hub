DROP FUNCTION IF EXISTS public.get_relatorio_convidados(uuid, timestamptz, timestamptz, uuid[]);
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN system_feature_flags sff ON sff.id = ffe.feature_flag_id
    WHERE sff.flag_key = 'relatorio_leads_convidados'
      AND ffe.empresa_id = p_empresa_id
      AND ffe.enabled = true
  ) THEN
    RETURN;
  END IF;
