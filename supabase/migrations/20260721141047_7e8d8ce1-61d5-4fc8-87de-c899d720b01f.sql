          ...
          COALESCE((
            ...
            FROM (
              SELECT DISTINCT ON (id) id, lead_id, nome, telefone, email, status,
                     prospeccao_id, responsavel_email, observacoes, origem, created_at, updated_at,
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'contatos', v_contatos,
    'total', v_total,
    'page', CASE WHEN p_limit > 0 THEN floor(p_offset::numeric / p_limit::numeric)::integer ELSE 0 END,
    'pageSize', p_limit
  );
END;
$function$;