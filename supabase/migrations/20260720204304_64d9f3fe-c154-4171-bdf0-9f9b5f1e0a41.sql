-- Front B: get_kanban_columns_limited passa a usar status por evento
-- Mantém: filtro de equipe (prospeccao_equipe_membros), filtro por responsável
-- em colunas não-Novo, filtros de data/busca, DISTINCT ON, LIMIT.
-- Muda: c.status::text = v_status  ->  get_contato_status_por_evento(c.id, ep.prospeccao_id) = v_status
-- Rollback: recriar versão anterior (backup em migração 20260413173538).
