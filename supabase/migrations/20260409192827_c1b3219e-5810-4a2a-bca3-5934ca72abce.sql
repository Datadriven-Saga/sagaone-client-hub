...
-- 1) Recreate get_kanban_columns_limited with team membership filter
CREATE OR REPLACE FUNCTION public.get_kanban_columns_limited(p_empresa_id uuid, p_per_column integer DEFAULT 20, p_prospeccao_id uuid DEFAULT NULL::uuid, p_responsavel text DEFAULT NULL::text, p_search text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
-- 2) Recreate auto_atribuir_leads_vendedor with team membership filter
