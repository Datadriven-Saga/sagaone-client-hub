...
BEGIN
  ...
  -- PERF: pre-carregar profiles da empresa em temp table indexada por chave
  -- (email, id, celular). Elimina join profiles⨝auth.users por linha.
  ...
  SELECT DISTINCT ON (key) key, nome_completo FROM (
    ...
    UNION ALL
    SELECT p.id::text AS key, p.nome_completo
      FROM public.profiles p
     WHERE p.empresa_id = p_empresa_id
    UNION ALL
    SELECT p.celular AS key, p.nome_completo
      FROM public.profiles p
  ...
  LOOP
    BEGIN
      ...
      DO UPDATE SET
        ...
        END,
        vendedor_nome = CASE
          WHEN p_force_status_novo AND (EXCLUDED.responsavel_email IS NULL OR EXCLUDED.responsavel_email = '')
            THEN NULL
      ...
      -- PERF: lookup via temp table (1 query/row) em vez de UPDATE+EXISTS com join auth.users (2 queries/row)
      IF v_responsavel IS NOT NULL AND v_responsavel <> '' THEN
        v_vendedor_nome := NULL;
        v_found_profile := false;
        SELECT nome_completo INTO v_vendedor_nome
        ...
        END IF;
      END IF;
      -- NB: limpeza de vendedor_nome em reimportação sem responsável já é feita no ON CONFLICT acima.
