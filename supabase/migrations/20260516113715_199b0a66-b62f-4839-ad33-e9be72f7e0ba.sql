-- Refatoração da função de encerramento de eventos com parâmetros de mitigação
...
BEGIN
  ...
  LOOP
    ...
    descarte_count := v_descarte_count;
    RETURN NEXT;
...
-- Reagendar cron com limite de 50 eventos/dia (eventos recém-encerrados, com descarte normal)
SELECT cron.unschedule('encerrar-eventos-finalizados') WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'encerrar-eventos-finalizados'
);

SELECT cron.schedule(
  'encerrar-eventos-finalizados',
  '0 6 * * *',
  $$SELECT public.encerrar_eventos_finalizados(50, NULL, false);$$
);