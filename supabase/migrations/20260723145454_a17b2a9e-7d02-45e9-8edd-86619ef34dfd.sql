...
BEGIN
  ...
  status_por_par AS (
    SELECT p.*,
           CASE
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('novo', 'novos') THEN 'Novo'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('atribuído', 'atribuidos') THEN 'Atribuído'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('em espera', 'emespera') THEN 'Em Espera'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('convidado', 'convidados') THEN 'Convidado'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('confirmado', 'confirmados') THEN 'Confirmado'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('check-in', 'checkin') THEN 'Check-in'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('venda', 'vendas') THEN 'Venda'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('descartado', 'descartados') THEN 'Descartado'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('opt out', 'optout') THEN 'Opt Out'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('desperdício', 'desperdicio') THEN 'Desperdício'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('agendado', 'agendados') THEN 'Agendado'
             WHEN LOWER(COALESCE(lm.status_novo, p.status_global, 'Novo')) IN ('negociação', 'negociacao') THEN 'Negociação'
             ELSE COALESCE(lm.status_novo, p.status_global, 'Novo')
           END AS status
    FROM pares p
    LEFT JOIN LATERAL (
  ...
  ranked AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY status ORDER BY updated_at DESC, id) AS rn
    FROM status_por_par
  ),
  items AS (
    SELECT status,
    ...
    FROM ranked
    WHERE rn <= p_per_column
    GROUP BY status
  )
  SELECT COALESCE(
    jsonb_object_agg(
      t.status,
      jsonb_build_object('count', t.cnt, 'items', COALESCE(i.items, '[]'::jsonb))
    ),
    '{}'::jsonb
  )
  INTO v_result
  FROM totals t
  LEFT JOIN items i ON i.status = t.status;
