  ...
  LOOP
    BEGIN
      ...
      IF p_prospeccao_id IS NOT NULL AND v_contato_id IS NOT NULL THEN
        -- Check if already linked to avoid counting duplicates
        ...
        IF NOT v_is_teste AND v_quarentena_enabled AND v_marca IS NOT NULL THEN
          INSERT INTO public.contato_quarentena (
            telefone_normalizado, marca, canal, empresa_id,
            prospeccao_id, evento_nome, data_fim_evento, ultimo_impacto_at
          ) VALUES (
            v_telefone_raw, v_marca, p_canal, p_empresa_id,
            p_prospeccao_id, v_evento_nome, v_data_fim_evento, now()
          )
          ON CONFLICT (telefone_normalizado, marca, canal)
          DO UPDATE SET
