...
BEGIN
  ...
  ultimo_log AS MATERIALIZED (
    ...
    ORDER BY l.prospeccao_id, l.contato_id, l.data_movimentacao DESC NULLS LAST, l.created_at DESC
  ),
...
GRANT EXECUTE ON FUNCTION public.normalize_lead_status_label(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_diagnostico_status_filtros() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_leads_status_divergente(uuid[], uuid[], text[], text[], text, timestamp with time zone, timestamp with time zone, integer, integer) TO authenticated, service_role;
REVOKE EXECUTE ON FUNCTION public.get_diagnostico_status_filtros() FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_leads_status_divergente(uuid[], uuid[], text[], text[], text, timestamp with time zone, timestamp with time zone, integer, integer) FROM anon;