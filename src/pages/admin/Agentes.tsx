...
export default function AdminAgentes() {
  ...
  const carregarEmpresasAtribuidas = async (agenteId: string) => {
    try {
      ...
      const { data, error } = await supabase.from("agente_empresas").select("empresa_id").eq("agente_id", agenteId);
  ...
  const buscarAgenteLocal = async (
    telefone: string,
    agenteWebhook?: AgenteWebhook | null,
  ): Promise<AgenteLocal | null> => {
    try {
      const { data, error } = await supabase.from("agentes_ia").select("*").eq("telefone", telefone).maybeSingle();
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("agent-photos").upload(filePath, file);
  ...
  const handleSaveAgente = async () => {
    ...
    const buildAtualizaAgentePayload = () => {
      ...
      const numeroAgente =
        ...
        instanciaFonte?.num_maia ||
        selectedAgente?.num_maia ||
        selectedAgente?.telefone ||
        "";
    ...
    try {
      ...
      if (agenteLocal) {
        ...
        const { data: updatedAgent } = await supabase.from("agentes_ia").select("*").eq("id", agenteLocal.id).single();
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("agentes_ia").update({ ativo: newStatus }).eq("id", agenteLocal.id);
  ...
  const handleDeleteAgente = async () => {
    ...
    try {
      const { error } = await supabase.from("agentes_ia").delete().eq("id", agenteLocal.id);
  ...
  const handleCriarInstancia = async () => {
    ...
    try {
      ...
      const nomeAgente =
        formData.nome.trim() ||
        novaInstancia.agente.trim() ||
        `${novaInstancia.agente.trim() || "Maia"} ${novaInstancia.marca.trim()} ${novaInstancia.uf.trim()}`;
  ...
  const handleAssignAgente = async () => {
    ...
    try {
      ...
      const { error: deleteError } = await supabase.from("agente_empresas").delete().eq("agente_id", agenteId);
      ...
      if (selectedEmpresaIds.length > 0) {
        ...
        const { error: assignError } = await supabase.from("agente_empresas").insert(atribuicoes);
    ...
    return (
      <DashboardLayout title="Agentes - Administração">
        <Alert variant="destructive">
          <AlertDescription>
            Acesso restrito. Apenas usuários do departamento TI com tipo de acesso TI ou Administrador podem acessar
            esta página.
          </AlertDescription>
        </Alert>
      </DashboardLayout>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Gerencie agentes de IA e controle de implantação</p>
              </div>
            </div>
              ...
              <Card>
                ...
                <CardContent>
                  {loading ? (
                    ...
                    <p className="text-muted-foreground text-center py-8">Nenhum agente encontrado</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow>
                        ...
                        {paginatedAgentes.map((agente, index) => (
                          ...
                          >
                            ...
                            <TableCell className="text-right">
                              <Button size="sm" variant="outline" onClick={(e) => handleOpenAssignModal(e, agente)}>
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-between mt-4 pt-4 border-t">
                      <p className="text-sm text-muted-foreground">
                        Mostrando {startIndex + 1}-{Math.min(endIndex, filteredAgentes.length)} de{" "}
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
                      <TabsList className="w-max sm:w-auto flex-nowrap">
                        ...
                        <TabsTrigger value="qualificacao" disabled={!agenteLocal}>
                          Qualificação
                        </TabsTrigger>
                        <TabsTrigger value="cadencia" disabled={!agenteLocal}>
                          Cadência
                        </TabsTrigger>
                        <TabsTrigger value="periodo" disabled={!agenteLocal}>
                          Jornada da IA
                        </TabsTrigger>
                      ...
                      activeTab !== "eventos" && (
                        <div className="flex items-center justify-center py-12 text-center">
                          <div className="space-y-2">
                            ...
                            <Button onClick={() => setActiveTab("dados-gerais")}>Voltar para Dados Gerais</Button>
                          </div>
                        </div>
                      )}
                            ...
                            <div className="mt-6 pt-6 border-t">
                              <div className="flex items-center justify-between gap-2 mb-4">
                                <div className="flex items-center gap-2">
                                  ...
                                  {loadingLojas && <RefreshCw className="h-4 w-4 animate-spin text-muted-foreground" />}
                                ...
                                {lojasAtribuidas.length > 0 && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    >
                                      ...
                                      filteredLojasAtribuidas.every((l) => selectedLojasToRemove.includes(l.id))
                                        ? "Desmarcar Todas"
                                        : "Selecionar Todas"}
                                    </Button>
                              ...
                              {lojasAtribuidas.length > 0 ? (
                                filteredLojasAtribuidas.length > 0 ? (
                                  ...
                                  <div className="text-center py-6 bg-muted/30 rounded-lg border border-dashed">
                                    ...
                                    <Button variant="link" size="sm" onClick={resetLojaFilters} className="mt-2">
                                      Limpar filtros
                                    </Button>
                                  </div>
                                )
                                ...
                                <CardContent className="p-4">
                                  <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      <div className="space-y-1">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                          Número do WhatsApp do agente (apenas dígitos)
                                        </p>
                                      </div>
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                          Marca/fabricante associado ao agente
                                        </p>
                                      </div>
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                          ID da caixa de entrada no Chatwoot
                                        </p>
                                      </div>
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                          Nome da tabela para guardar histórico de conversas
                                        </p>
                                      </div>
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      />
                                      <p className="text-xs text-muted-foreground">
                                        Token de acesso do agente no Chatwoot
                                      </p>
                                    </div>
                                    ...
                                    {instanciaPreview && (
                                      <div className="p-3 bg-muted rounded-lg">
                                        <Label className="text-xs text-muted-foreground">
                                          Instância gerada automaticamente:
                                        </Label>
                                    ...
                                    <div className="flex gap-3 pt-4 border-t">
                                      <Button onClick={handleCriarInstancia} disabled={creatingInstancia}>
                                        {creatingInstancia ? (
                                          <>
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                          Identificador único da instância Evolution
                                        </p>
                                      </div>
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    {instanciaData.cw_token_maia && (
                                      <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                          <div className="flex-1 min-w-0 font-mono text-xs bg-muted p-2 rounded break-all overflow-hidden">
                                            {showCwToken ? instanciaData.cw_token_maia : "••••••••••••••••••••••••"}
                                          </div>
                                          <Button
                                            size="sm"
                                            variant="ghost"
                                          ...
                                          <Button
                                            ...
                                            variant="ghost"
                                            onClick={(e) =>
                                              handleCopyToClipboard(e, instanciaData.cw_token_maia!, "CW Token Maia")
                                            }
                                            title="Copiar"
                                          >
                                            <Copy className="h-4 w-4" />
                    ...
                    {isPriLigacao() && (
                      <TabsContent value="eventos">
                        <AgenteEventos agenteId={agenteLocal?.id} agenteTelefone={formData.telefone} />
                      </TabsContent>
                    )}
              ...
              <Dialog open={showAssignModal} onOpenChange={setShowAssignModal}>
                <DialogContent className="w-[95vw] sm:max-w-2xl max-h-[85vh] flex flex-col">
                    ...
                    <div className="flex items-center justify-between pb-2 border-b flex-shrink-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          ...
                          filteredEmpresasModal.every((e) => selectedEmpresaIds.includes(e.id))
                            ? "Desmarcar todas"
                            : "Selecionar todas"}
                        </Button>
                        <Badge variant="secondary">{selectedEmpresaIds.length} selecionada(s)</Badge>
                      </div>
                    </div>
                  ...
                  <DialogFooter className="flex-shrink-0 pt-4 border-t">
                    ...
                    <Button onClick={handleAssignAgente}>Confirmar ({selectedEmpresaIds.length})</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </TabsContent>
