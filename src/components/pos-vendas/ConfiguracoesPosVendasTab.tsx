...
// ---------- Types ----------
type Loja = { dealer_id: number; marca: string; uf: string };
...
const MOCK_LOJAS: Loja[] = [
  { dealer_id: 101, marca: "VW", uf: "DF" },
  { dealer_id: 102, marca: "HONDA", uf: "GO" },
  { dealer_id: 103, marca: "TOYOTA", uf: "SP" },
  { dealer_id: 104, marca: "BMW MOTOS", uf: "DF" },
];
// Lojas que "possuem" configuração salva. As demais caem no default + banner.
const LOJAS_COM_CONFIG = new Set<number>([101, 103]);
...
function isDirty(a: ConfigState, b: ConfigState) {
  return JSON.stringify(a) !== JSON.stringify(b);
}

export function ConfiguracoesPosVendasTab() {
  const [lojas] = useState<Loja[]>(MOCK_LOJAS);
  ...
  const lojaSelecionada = useMemo(
    () => lojas.find((l) => l.dealer_id === dealerId) ?? null,
    [lojas, dealerId]
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const t = setTimeout(() => {
      const semConfig = !LOJAS_COM_CONFIG.has(dealerId);
      const base = cloneConfig(DEFAULT_CONFIG);
      // Se tiver "config salva", só bagunço um pouco pra diferenciar do default
      if (!semConfig) {
        base.tarde.slots = 4;
        base.antecedencia_dias = 45;
      }
      setConfig(base);
      setOriginal(cloneConfig(base));
      setUsouDefault(semConfig);
      setLoading(false);
    }, 350);
    return () => clearTimeout(t);
  }, [dealerId]);

  const dirty = !!(config && original && isDirty(config, original));
  ...
  function validarTudo(c: ConfigState): string | null {
  ...
  function handleCancelar() {
    if (!original) return;
    setConfig(cloneConfig(original));
  }

  async function handleSalvar() {
    if (!config || !lojaSelecionada) return;
    const err = validarTudo(config);
    if (err) {
      toast.error(err);
      return;
    }
    ...
    try {
      ...
      const payloadConfig = {
        dealer_id: lojaSelecionada.dealer_id,
      ...
      const [r1, r2] = await Promise.all([
        fetch(`${WEBHOOK_BASE}/upsert-paty-posvendas-config`, {
          method: "POST",
          headers,
          body: JSON.stringify(payloadConfig),
        }),
        fetch(`${WEBHOOK_BASE}/upsert-paty-revision-range`, {
          method: "POST",
          headers,
          body: JSON.stringify(payloadRanges),
        }),
      ]);
      if (!r1.ok || !r2.ok) throw new Error(`Falha ao salvar (${r1.status}/${r2.status})`);

      setOriginal(cloneConfig(config));
    ...
    <div className="space-y-4 pb-24">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="max-w-sm">
            <Label className="text-xs text-muted-foreground">Selecione a loja</Label>
            <Select
              value={dealerId?.toString() ?? ""}
              onValueChange={(v) => setDealerId(Number(v))}
            >
              ...
              <SelectContent>
                {lojas.map((l) => (
                  <SelectItem key={l.dealer_id} value={l.dealer_id.toString()}>
                    {l.marca} - {l.uf}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {!dealerId && (
            <p className="text-sm text-muted-foreground">
              Selecione uma loja para carregar ou configurar o pós-vendas.
            </p>
          )}
          {lojaSelecionada && !loading && (
            <p className="text-xs text-muted-foreground">
              Configurando: <span className="text-foreground font-medium">{lojaSelecionada.marca} - {lojaSelecionada.uf}</span> (dealer {lojaSelecionada.dealer_id})
            </p>
          )}
        </CardContent>
      </Card>

      {usouDefault && lojaSelecionada && (
      ...
      {config && !loading && (
        <>
          ...
          {/* Footer sticky */}
          <div className="fixed bottom-0 left-0 right-0 border-t border-border bg-background/95 backdrop-blur px-4 py-3 flex justify-end gap-2 z-30">
            <Button variant="ghost" onClick={handleCancelar} disabled={!dirty || saving}>
              Cancelar
            </Button>
            <Button
              onClick={handleSalvar}
              disabled={!dirty || saving || blocksDisabled}
              className={savedFlash ? "bg-emerald-600 hover:bg-emerald-600 text-white" : ""}
            >
              {saving ? (
